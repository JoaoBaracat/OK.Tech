...
namespace OK.Tech.Api.Controllers
{
    ...
    public class PriceListController : MainController
    {
        ...
        {
            var priceLists = _mapper.Map<IEnumerable<PriceListViewModel>>(await _priceListApp.GetAll());

            return CustomResponse(priceLists);
        }

        [HttpGet("{id:Guid}")]
        public async Task<ActionResult<PriceListViewModel>> GetPriceListById(Guid id)
        {
            var priceList = _mapper.Map<PriceListViewModel>(await _priceListApp.GetById(id));

            return CustomResponse(priceList);
        }
